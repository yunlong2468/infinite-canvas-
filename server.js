...
function executeToolAsync(toolName, argsJson, projectId, userId, streamCallback) {
    return new Promise(function(resolve) {
        ...
        if (tl.indexOf('outline') >= 0 || toolName === 'generate_outline') {
            ...
            if (userTool) {
                ...
                // 动态工具身份前缀：告知LLM它不是调配师，是专项工具子智能体
                ...
                // 构建工具列表描述（给 request_tool 枚举可用工具）
                ...
                var requestToolDef = { type: 'function', function: { name: 'request_tool', description: '向主智能体请求调用工具。可用工具：'+toolListStr, parameters: { type: 'object', properties: { tool_name: { type: 'string', description: '要请求调用的工具名（从可用工具列表中选择）' }, tool_args: { type: 'object', description: '工具参数JSON对象' } }, required: ['tool_name'] } } };
                callOutlineLLM(projectId, userId, toolSystemPrompt, toolContext, toolName, null, function(result) {
                    if (result.error) { resolve({ error: result.error, summary: '动态工具执行失败: '+result.error }); return; }
                    resolve({ result: result.content, thinking: result.thinking || '', summary: '工具「'+toolName+'」执行完成' });
                }, streamCallback, [requestToolDef]);
            } else {
