...
'- 调用crawl_books前必须让用户明确说出平台名称，严禁从上下文推断。如果用户只说"末世""科幻"等类型未提平台，必须追问平台。\n'+
...
function executeToolAsync(toolName, argsJson, projectId, userId) {
    return new Promise(function(resolve) {
        ...
        } else if (toolName === 'generate_characters') {
        ...
        while (toolLoopCount < 5) { // 最多5轮工具调用防无限循环
            ...
            if (toolMsg.tool_calls && toolMsg.tool_calls.length > 0) {
                ...
                for (var ti = 0; ti < toolMsg.tool_calls.length; ti++) {
                    ...
                    var subAgentType = toolName==='generate_outline'?'outliner':toolName==='generate_characters'?'character':toolName==='crawl_books'?'crawler':toolName;
                    ...
                    var platformSuffix = (toolName === 'crawl_books' && platform) ? '（平台：'+platform+'）' : '';
                    var inviteMsg = '{agent:orchestrator}调用{agent:'+subAgentType+'}智能体'+platformSuffix;
